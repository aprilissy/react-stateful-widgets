...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const nameArr = programmersList.filter(dev => featuredProgrammerId === dev.id)
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeaturedProgrammerId(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
